...
- `src/config/affiliate.ts` holds the two Amazon Associates tracking IDs (`bushgums-kit-22` for landing page kit links, `bushgums-gear-22` for gear detail links). `src/lib/affiliate-tracking.ts` canonicalises Amazon product URLs to `/dp/{ASIN}` and applies the correct tag at render time, so offers store clean product URLs with no hard-coded tag. See `docs/affiliate-links.md`.
